...
function BattleshipCFB(team, year, yearend) {
  ...
  let refreshID = setInterval(function() {
    ...
    rp("https://en.wikipedia.org/wiki/" + year + "_" + team +"_football_team")
      .then((html) => {
        ...
        if (record.length > 0 && typeof record !== 'undefined') {
          ...
          for (let h = 1; h < record.length - 1; h++) { //FOR EACH RECORD IN YEAR
            ...
            var vard = leftpad((date.split(' ')[1]), 2, "0");

            var vary = year-1

            if (varm == '01') { vary = vary + 1}
...
var year = 1906;
var yearend = 1910;
BattleshipCFB(team, year, yearend);
